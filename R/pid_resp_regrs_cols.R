...
pid_resp_regrs_cols <- function(df, pid, n_cov, n_resp, regr_vec=NULL,
    ...
    grp_cols <- if (is.null(group)) NULL else var_cols(df=df, var_name=group)[[group]]
    ...
    list(pid_cols=pid_cols, pw_cols=pw_cols, resps_cols=resps_cols, regr_ls=regr_ls, grp_cols=grp_cols)
}
