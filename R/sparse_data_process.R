#' sparse_data_process
#'
#' This function preprocesses data in three ways. First, convert non-used categories and missing values to NA. Second, remove item responses with all missing values. Third, for DIF analysis, remove items that have no data on any category of the DIF variable. This is associated with test named 'test'. An Excel file with processed data and removed items will be saved to 'data' folder in working directory.
#'
...
#' @return List of processed data, updated dimension test numbers, updated keys, and updated labels.
#' @examples
#' processed <- sparse_data_process(test='Dan', data=df_DIF, keys=rep(1,1072), labels=rep(1,1072), n_cov=5, n_dims=c(901, 171), miss_code=c('.', 'r', 'R', 'x', 'X', '', ' '), DIFVar='studGender')
#' processed1 <- sparse_data_process(test='Dan', data=df_DIF, keys=rep(1,1072), labels=rep(1,1072), n_cov=5, n_dims=c(901, 171), miss_code=c('.', 'r', 'R', 'x', 'X', '', ' '))
#' processed_elena <- sparse_data_process(test='elena', data=elena, keys=rep(1,39), labels=rep(1,39), n_cov=9, n_dims=c(30, 9), miss_code=c('.', 'r', 'R', 'x', 'X', '', ' '), DIFVar='gender')
#' @export
