...
#' @examples
#' delta_DIF_dich(test='elena_facet', DIFVar='quintile')

delta_DIF_dich <- function(folder=NULL, test, DIFVar){
    if (is.null(folder)) folder <- here::here('DIF', DIFVar)
    n_item <- N_item(folder=folder, test=test)

    delta_its <- file_its(folder=folder, test=test, DIFVar=DIFVar) %>%
    ...
    delta_its %>%
        filter(var==1) %>%
        rename(delta.x=delta) %>%
        cbind(delta_its %>%
              filter(var==2) %>%
              rename(delta.y=delta)) %>%
        select(-var)
}
