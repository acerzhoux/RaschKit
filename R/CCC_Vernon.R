...
#' @examples
#' plot_data <- CCC_Vernon(test='elana_poly_score', abilEst2use='pv1')
#' plot_data <- CCC_Vernon(test='racp1', abilEst2use='pv1')
#' @export

CCC_Vernon <- function(folder=here::here('output'), test, abilEst2use='pv1',
                       numAbilGrps=NULL, long_label=FALSE){
    thr <- df_thr(folder=folder, test=test, long_label=long_label)
    cqs <- conquestr::ConQuestSys(file.path(folder, paste0(test, ".CQS")))
    ...
    if(2 %in% unique(iEstTemp$modelTerm)){
      constrained2 <- iEstTemp %>% filter(modelTerm > 1) %>%
    ...
    deltas <- iStepsCounts %>% filter(iStepsCount > 0) %>%
      ...
      left_join(iEstimates %>% filter(modelTerm == 2) %>%
                  rename(stepDelta = Xsi, stepError = error),
                by = "iLab")  %>%
      select(-modelTerm) %>%
    ...
    iFitStats <- iStepsCounts %>% filter(iStepsCount > 0) %>%
      rowid_to_column() %>%
      left_join(cqs$gFitStatistics$Value %>% {
    ...
    catStats <- bind_cols(
      ...
      filter(count > 0) %>%
      left_join(keys %>% rename(resp = key),
                by = c("iNum", "resp")) %>%
      group_by(iNum) %>%
    ...
    iStats <- catStats %>%
      ...
      group_by(iNum) %>%
      summarise(Facility = sum(scorexpercent)/max(score))  %>%
      ...
      left_join(
        cqs$gMatrixList$i_itemtotrestcor %>% as_tibble() %>%
          rename(`Item-Rest` = `item-rest`, `Item-Total` = `item-total`) %>%
          rowid_to_column("iNum"),
    ...
    if (is.null(numAbilGrps)) numAbilGrps <- map(ccDat, ~round(log10(nrow(.))+1))
    ...
    dfObs_opt <- ccDat_df %>%
        filter(respPrekey < 9, respPrekey > 0) %>%
    ...
    ccc_dich <- function(iStats, dfObs_opt, i){
        ...
        ggplot() +
            ...
            labs(title = paste0("Category Characteristic Curve\n", "Item: ", i, " (",
                                i_df$iLab,")"),
                 subtitle = paste0("Weighted MNSQ = ", round(i_df$WeightedMNSQ,2)),
                 x = "Latent Train (logits)", y = "Probability") +
    ...
    ccc_poly <- function(iStats, dfObs, i, dfModel){
        ...
        kkk <- tibble(iNum = i, ability = seq(xMin, 3, by=.02)) %>%
    ...
    plot_ls <- list()
    ccc_data_ls <- list()
    for(i in unique(iStats$iNum)){
        if (itype_condition$itype[[i]] %in% 'dich'){
            plot_ls[[i]] <- ccc_dich(iStats=iStats, dfObs_opt=dfObs_opt, i=i)
            ccc_data_ls[[i]] <- dfObs_opt %>% filter(iNum == i)
        } else if (itype_condition$itype[[i]] %in% 'poly'){
            plot_ls[[i]] <- ccc_poly(iStats=iStats, dfObs=dfObs, i=i, dfModel=dfModel)
        } else if (itype_condition$itype[[i]] %in% 'score'){
            plot_ls[[i]] <- ccc_score(iStats=iStats, dfObs=dfObs, i=i)
            ccc_data_ls[[i]] <- dfObs %>% filter(iNum == i) %>%
                mutate(Category=ifelse(Category==1, str_c(1, '*'), 0)) %>%
                rename(Option=Category)
        }
    }
