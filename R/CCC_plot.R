...
CCC_plot <- function(easy=90, hard=10, iRst=.11, fit_w=1.1, fit_uw=1.2){
  ...
  testStatPlot <- function(i, iStats) {
    i_df <- iStats |>
      filter(iNum == i) |>
      select(
        Delta=iLogit,
    ...
    row_fitFlag <- with(i_df, Infit>fit_w | Outfit>fit_uw)
  ...
  optStatPlot <- function(i, tblInt) {
    ...
    tblStats <- tblStats |>
      dplyr::mutate(
        ...
        key_abil = tblStats[str_detect(tblStats$Option, "\\*"), ]$PV1Avg,
        unexp_ptbis = ifelse(`% Total`>10 & `Pt Bis`>0 & key_abil!=PV1Avg, 1, 0),
        unexp_abil = ifelse(`% Total`>10 & PV1Avg>key_abil, 1, 0)
      )
    ...
    tblStatsFlex <- tblStats |>
      ...
      flextable::flextable() |>
      flextable::bg(i = row_key, bg = "yellow", part = "body") |> # purple
  ...
  ccc_poly <- function(iStats, dfObs, i, dfModel, tblInt){
    ...
    ggplot() +
      ...
      labs(title = paste0("Category Characteristic Curves\n", "Item: ", i, " (",
                          i_df$iLab,")"),
           subtitle = paste0("Weighted MNSQ = ", i_df$WeightedMNSQ,
                             "\nDelta(s) ",
