...
#' @param steps Whether polytomous items are involved.
#' @return Plot of one category's facilities vs. those of the other category of the dichotomous DIF variable.
#' @examples
#' plot_facil()
#' @export

plot_facil <- function(test, facil_dif, vars, facil_cut, steps, DIFVar){
    ...
    ggplot(facil_dif, aes(`1`, `2`)) +
        ...
        labs(title=paste0(str_to_title(test), ': Item ',
                          if(steps) 'step ', 'Facility Comparison on ', str_to_title(DIFVar)),
