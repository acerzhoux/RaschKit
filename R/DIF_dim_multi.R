...
DIF_dim_multi <- function(method=c('chi_square', 'Bonferroni'), test, DIFVar,
  ...
  if (prep_process){
    ...
    processed <- sparse_data_process(test=test, data=data, keys=keys, labels=labels,
                  n_cov=n_cov, n_dims=n_dims, miss_code=c('.', 'r', 'R', 'x', 'X', '', ' '),
                  DIFVar=DIFVar)
    data <- processed[['data']]
  ...
  prep <- df_key_lab_args(test, data, pid, n_cov, sum(n_dims), DIFVar,
  ...
  arg_cqc <- list(test=test, run=NULL, run_ls=NULL, keys=keys,
    ...
    anchor=FALSE, step=step, regr_ls=prep$regr_ls, section_extr=prep$section_extr,
