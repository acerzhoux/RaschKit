...
delta_DIF_dich_step <- function(folder=NULL, test, DIFVar, quick=TRUE){
    ...
    # delta
    x <- readLines(file_path_type(folder=folder, test=test, type='itn'))
    ind <- grep('Item Delta\\(s\\)', x)
    x <- map(x[ind], ~str_sub(.x, 15, -1) %>%
                  str_squish() %>%
                  str_split(' ')) %>%
        map(1) %>%
        map(as.numeric)

    deltas_ls <- list(map2(x[2*(1:n_item)-1], labs$qid,
                        ~tibble(item=str_c(.y, '_', 1:length(.x)),
                                delta=.x)) %>%
                       reduce(bind_rows),
    ...
    if (quick){
        MofM1 <- map_dbl(x[2*(1:n_item)-1], mean) %>%
            mean
        MofM2 <- map_dbl(x[2*(1:n_item)], mean) %>%
            mean
    ...
    # error
    file_shw <- file_path_type(folder=folder, test=test, type='shw')
    y <- readLines(file_shw)
    ind1 <- grep('TERM 4\\: item\\*step\\*', y)[[2]]+5
    ind2 <- grep('An asterisk next to', y)[[4]]-2

    errors <- read_fwf(file_shw,
             fwf_cols(qOrder=c(2, 5),
                      step=c(19, 26),
                      DIFVar=c(35, 40),
                      error=c(61, 65)),
             skip=ind1,
