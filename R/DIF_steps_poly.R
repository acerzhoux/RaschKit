...
DIF_steps_poly <- function(){
  ...
  Details = c('Separately calibrate all items for each category of the DIF variable.',
    ...
    'Consider the standardized estimate as Bonferroni adjusted normal distribution ~ N(0, 1) with degree of freedom of n(n-1)/2 where n is the number of categories of the DIF variable. Then, the p value of the two-tailed significant test will be 0.05 divided by n(n-1)/4. The standardized deviation is significant when its absolute value is bigger than the z value at the adjusted p level.'))
}
