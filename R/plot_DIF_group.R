...
plot_DIF_group <- function(test, DIFVar, numAbilGrps=3, abilEst2use="pv1"){
  ...
  pStats <-  tibble(
    ...
    pvs = map(cqs$gAllCaseEstimates, "pvs"),
  ...
  preKeyLookUp <- cqs$gPreKeyLookUp |> tibble::as_tibble() |>
    ...
    dplyr::mutate(respPrekey = rowid-1) |>
  ...
  # ICCs by var_group - Using .data pronoun for safer column reference
  dfObserved <- ccDat |>
    ...
    group_by(iNum, group, Ability = abilityGrp, Score = score, var_group) |>
    dplyr::summarise(Count = n()) |> ungroup() |>
    complete(Score, nesting(iNum, group, var_group, Ability), fill = list(Count = 0)) |>
    group_by(iNum, Score) |>
    dplyr::mutate(scoreCount = sum(Count)) |>
    dplyr::filter(scoreCount > 0) |>
    dplyr::select(-scoreCount) |>
    group_by(iNum, Ability, var_group, group) |>
    dplyr::mutate(prop = Count/sum(Count)) |>
    ungroup() |>
    arrange(iNum, group, Ability, Score) |>
    dplyr::mutate(var_group = str_pad(var_group, 2, "left", "0")) |>
    # Rename the column to match DIFVar at the end for plotting
    dplyr::rename({{ DIFVar }} := var_group)
  ...
  for(k in unique(deltas$iNum)){
    ...
    o_df <- dfObserved |>
      dplyr::filter(iNum == k, Score == 1, !is.na(.data[[DIFVar]])) |>
      bind_rows(dfObservedAverage |>
              dplyr::filter(iNum == k, Score == 1) |>
              dplyr::mutate({{ DIFVar }} := "Average\nExpected")) |>
    ...
    colourCount = length(unique(o_df[[DIFVar]]))
    getPalette = colorRampPalette(brewer.pal(9, "Set1"))

    plot <- ggplot() +
      geom_line(m_df, mapping= aes(x=ability, y=pSuccess, linetype = lineLab), lwd = 1) +
      geom_point(o_df, mapping = aes(x = Ability, y = prop,
                       shape = .data[[DIFVar]],
                       fill = .data[[DIFVar]],
                       colour = .data[[DIFVar]]), size = 4)  +
      geom_line(o_df, mapping = aes(x = Ability, y = prop,
                      colour = .data[[DIFVar]],
