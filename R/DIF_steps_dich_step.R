...
DIF_steps_dich_step <- function(iterative=FALSE){
  tibble(
    Details = c('Multiply each step estimate\'s error in both groups by sqrt(desig_effect).',
      ...
      if (iterative){
          'Iteratively perform Step 5 and each time remove the step of largest absolute adjusted step difference until steps all satisfy thresholds of p value, Chi-squared value, step difference, and adjusted step difference.'
      }),
