...
itn_comment <- function(){
  tibble(
    ...
    Description=c(
      ...
      'Additional items flagged after reviewing Category Characteristic Curve (CCC) plots for any potential issues for dichotmous items, or after checking score category proportion distribution and ability estimates for polytomous items. Items listed in this fourth priority group alone are identified only on such basis.'
    )
  )
}
