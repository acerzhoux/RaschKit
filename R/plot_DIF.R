...
#' @param wh 'Before'/'After' when to plot before or after equating or in DIF analysis.
...
plot_DIF <- function(df, wh, vars, p_cut=0.05, chi_cut=10,
                     DIF_cut=0.5, DIF_adj_cut=4, steps=FALSE, DIF=FALSE) {

    if (steps){
        df <- df %>% select(-delta.x, -delta.y) %>%
            rename(item=iStep, delta.x=delta.x_dev, delta.y_adj=delta.y_dev)}
    ...
    p <- ggplot(df, aes(delta.x, delta.y_adj)) +
        ...
        labs(title=paste0(wh, ' Review: ', dim(df)[1], if (DIF) ' items' else ' anchors'),
    ...
    if (wh=='Before'){
        DIF_txt <- DIF_items(df=df, p_cut=p_cut, chi_cut=chi_cut,
            ...
            select(item, delta.x, delta.y_adj)
        p <- p +
