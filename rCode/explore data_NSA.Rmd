...
for (dm in c('b', 'm')){
  ...
  for (gr in c(3, 5)){
    deltaLst[[paste0(tst, '_', gr)]] <- readxl::read_xlsx('equating/hist_stats.xlsx', paste0(dm, 17, '_', gr)) |> 
...
for (dm in c('b', 'm')){
  tst <- ifelse(dm=='b', 'Bangla', 'Math') 
  for (gr in c(3, 5)){
    indiceLst[[paste0(tst, '_', gr)]] <- readxl::read_xlsx('equating/stats_17_22.xlsx', paste0(dm, 17, '_', gr)) |> 
      inner_join(
...
equate_lst(deltaLst, vars=c(17, 22), indiceLst=indiceLst)
```
