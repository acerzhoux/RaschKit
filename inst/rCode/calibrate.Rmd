...
resp <- readxl::read_xlsx(list.files(here::here('dataRaw'), 
        'data', # revise to unique string in file name
         full.names = TRUE)) %>% 
    map_df(~toupper(.x))
```
...
cd <- readxl::read_xlsx(list.files(here::here('dataRaw'), 
    ...
    row_to_names(1) 
keys <- cd %>% 
    pull(Key) # To be used as 'keys' for'calibrate' (below)
labels <- cd %>% 
    pull(Item) # To be used as 'labels' for'calibrate' (below)
```
...
- On the sheet, add columns below (row order same as response order in data).
  ...
  - Key: key for MC items. Polytomous items can fill in any value or be left blank.
    Can ignore if there is no MC item in test.
  - Max_score: Maximum score for MC and CR items. Simply, can use maximum item score 
    (after recoding) in the test for all polytomous items.
  - Key2: 2nd key. Can ignore if no double key.
