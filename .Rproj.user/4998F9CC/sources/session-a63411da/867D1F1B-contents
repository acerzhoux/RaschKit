...
equate_lst <- function(deltaDfLst, vars, p_cut=0.05, DIF_cut=0.5, DIF_adj_cut=4,
                       desig_effect=1, step=FALSE, iter=TRUE, indDfLst=NULL){
  # folders, file names
  subfolder <- ifelse(
      step,
  ...
  if (is.null(indDfLst)) {
    for (i in seq_along(tests)){
      Equate(deltaDfLst[[i]], tests[[i]], vars, p_cut, DIF_cut, DIF_adj_cut, TRUE,
           desig_effect, step, NULL, iter)
    }
  } else {
    for (i in seq_along(tests)){
      Equate(deltaDfLst[[i]], tests[[i]], vars, p_cut, DIF_cut, DIF_adj_cut, TRUE,
             desig_effect, step, NULL, iter, indDfLst[[i]])
    }
  }
